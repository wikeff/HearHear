...
import { processRecordings, uploadRecordingsToS3 } from './utils/audioProcessor';
 ...
 * @returns
 */
async function start(interaction: CommandInteraction, client: Client, connection?: VoiceConnection) {
	...
	try {
		...
		receiver.speaking.on('start', (userId) => {
			...
			if (interaction.member instanceof GuildMember && interaction.member.voice.channel) {
				const channel = interaction.member.voice.channel;
				createListeningStream(receiver, userId, channel.guild.id, client.users.cache.get(userId));
			}
		});
	} catch (error) {
		console.warn(error);
...
async function end(interaction: CommandInteraction, _client: Client, connection?: VoiceConnection) {
	if (connection) {
		...
		if (interaction.member instanceof GuildMember && interaction.member.voice.channel && interaction.guildId) {
			...
			await interaction.editReply({
				content: 'Failed to retrieve information needed to upload the files to S3. Need guildId and channelId!',
			});
		}
	} else {
...
export const interactionHandlers = new Map<
	string,
	(interaction: CommandInteraction, client: Client, connection?: VoiceConnection) => Promise<void>
>();
