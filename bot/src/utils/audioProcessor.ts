...
import { uploadMeetingFileToS3 } from './s3';
...
async function processRecordings(guildId: string) {
	...
	let files = fs.readdirSync(RECORDING_FILES_PATH + `/${guildId}`).filter((file) => file.endsWith('.ogg'));
	...
	files = fs.readdirSync(RECORDING_FILES_PATH + `/${guildId}`).filter((file) => file.endsWith('.ogg'));
	const convertLimit = pLimit(10);
	await Promise.all(
		files.map((file) =>
			convertLimit(async () => {
				if (!fs.existsSync(`${RECORDING_FILES_PATH}/${guildId}/${file.slice(0, -4)}.mp3`)) {
	...
	const mp3_files = fs.readdirSync(RECORDING_FILES_PATH + `/${guildId}`).filter((file) => file.endsWith('.mp3'));
			...
			async (file) =>
				await throttle(async () => {
					try {
						...
						const res = await transcribeAudio(`${RECORDING_FILES_PATH}/${guildId}/${file}`);
						...
						if (!(text === null || text === undefined || text.length === 0)) {
							fs.writeFileSync(`${RECORDING_FILES_PATH}/${guildId}/${file.slice(0, -4)}.txt`, text);
							console.log(`Successfully saved ${file.slice(0, -4)}.txt`);
						} else {
							console.log(`No text found for ${file.slice(0, -4)}.ogg: `, text, '\n', data);
							deleteFileSync(`${RECORDING_FILES_PATH}/${guildId}/${file.slice(0, -4)}.ogg`);
						}
					} catch (err) {
						console.log(err);
					}
...
async function mergeAudioFiles(files: string[], guildId: string) {
	...
	for (let i = 1; i < files.length; i++) {
		...
		if (fileUser === currentFileUser && fileDate.getTime() - currentFileDate.getTime() < 1_000_000) {
	...
	await Promise.allSettled(
		...
		merged_files.forEach((file) => {
			fs.renameSync(`${RECORDING_FILES_PATH}/${guildId}/merged/${file}`, `${RECORDING_FILES_PATH}/${guildId}/${file}`);
		});
	});
}
...
async function mergeAudioFilesHelper(files: string[], guildId: string) {
	...
	const fluent_ffmpeg = ffmpeg(`${RECORDING_FILES_PATH}/${guildId}/${files[0]}`);
	for (let i = 1; i < files.length; i++) {
	...
	await new Promise((resolve, reject) => {
		fluent_ffmpeg
			...
			.mergeToFile(`${RECORDING_FILES_PATH}/${guildId}/merged/${mergedFileName}`, './tmp');
	});
}

async function uploadRecordingsToS3(guildId: string, channelId: string) {
	const files = fs.readdirSync(RECORDING_FILES_PATH + `/${guildId}`).filter((file) => file.endsWith('.ogg'));
	...
	await Promise.all(
		files.map(async (file) => {
			fs.readFile(`${RECORDING_FILES_PATH}/${guildId}/${file}`, async (err, data) => {
				if (err) {
					console.log(err);
					return;
				}
				await uploadMeetingFileToS3(data, file, guildId, channelId, earliestDate)
					.then(() => {
						deleteFileSync(`${RECORDING_FILES_PATH}/${guildId}/${file}`);
					})
					.catch((err: any) => {
			...
			fs.readFile(`${RECORDING_FILES_PATH}/${guildId}/${file.slice(0, -4)}.txt`, async (err, data) => {
				if (err) {
					console.log(err);
					return;
				}
				await uploadMeetingFileToS3(data, file.slice(0, -4) + '.txt', guildId, channelId, earliestDate, 'text/plain')
					.then(() => {
						deleteFileSync(`${RECORDING_FILES_PATH}/${guildId}/${file.slice(0, -4)}.txt`);
					})
					.catch((err: any) => {
						console.log(err);
					});
